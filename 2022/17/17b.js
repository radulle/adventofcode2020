...
##`
  ...
  const canvas = [];
  const map = new Map();
  const ROUNDS = 1_000_000_000_000;
  ...
  round: for (let round = 0; round < ROUNDS; round++) {
    if (!addRepeatedLines) {
      const key =
        ...
        "." +
        canvas
          .slice(canvas.length - 42)
          .map((r) => r.join(""))
          .join("");

      if (map.has(key)) {
        const seen = map.get(key);
        const repeatEvery = round - seen.round;
        const repeatedRounds = Math.floor((ROUNDS - round) / repeatEvery);
        round += repeatedRounds * repeatEvery;
        addRepeatedLines = repeatedRounds * (canvas.length - seen.size);
      }
      map.set(key, { round, size: canvas.length });
    }
  ...
  console.info(canvas.length + addRepeatedLines);
}
