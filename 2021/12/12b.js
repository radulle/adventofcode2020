...
class CaveGraph extends Graph {
  ...
  dfs(start, finish) {
    ...
    const dfs = (sourceKey, visited = "", step = 0, double = false) => {
      for (const { key } of this.getAdjecent(sourceKey)) {
        ...
        if (/^[a-z]*$/.test(key) && visited.includes(key)) {
          if (double === true) continue;
          dfs(key, append(visited, key), step + 1, true);
          continue;
        }
        dfs(key, append(visited, key), step + 1, double);
      }
    };
