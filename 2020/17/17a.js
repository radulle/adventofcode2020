...
data.forEach((row, j) =>
  row.forEach(
    (cell, k) =>
      (cube[size / 2][size / 2 - data.length / 2 + j][
  ...
  for (let i = 1; i + 1 < size; i++) {
    for (let j = 1; j + 1 < size; j++) {
      for (let k = 1; k + 1 < size; k++) {
        if (
          cube[i][j][k] === 1 &&
          (count(cube, i, j, k) === 2 || count(cube, i, j, k) === 3)
        ) {
          newCube[i][j][k] = 1
        } else {
          newCube[i][j][k] = 0
        }
        if (cube[i][j][k] === 0 && count(cube, i, j, k) === 3)
          newCube[i][j][k] = 1
      }
    }
  }
...
function count(cube, z, y, x) {
  let num = 0
  for (let i = -1; i < 2; i++) {
    for (let j = -1; j < 2; j++) {
      for (let k = -1; k < 2; k++) {
        num += cube[z + i][y + j][x + k]
      }
    }
  }
  num -= cube[z][y][x]
