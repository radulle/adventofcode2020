...
const allergenFoods = [...allergens.values()].map((e) => e[0])

const safeFoods = data
  .map(({ food }) => food)
  .map((row) => row.filter((cell) => !allergenFoods.includes(cell)))

const result = safeFoods.reduce((acc, cur) => acc + cur.length, 0)
